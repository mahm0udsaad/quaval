...
import { quavalCatalogs } from "@/config/catalogs"

type QuavalCatalogsPageProps = {
  params: Promise<{ locale: string }>
}

export default async function QuavalCatalogsPage({ params }: QuavalCatalogsPageProps) {
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <h1 className="max-w-4xl text-4xl font-bold tracking-tight text-secondary sm:text-5xl">
              Quaval technical catalogs
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-text-light sm:text-lg">
              Browse Quaval bearing specifications online. Use the page and zoom controls to
              examine technical tables without downloading the source documents.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-black/10 bg-white px-5 py-4 shadow-sm">
            ...
            <div>
              ...
              <div className="text-sm text-text-light">catalogs available</div>
            </div>
          </div>
        </div>

        <div className="mt-10">
          <CatalogViewer catalogs={quavalCatalogs} />
        </div>
      </div>
    </div>
  )
