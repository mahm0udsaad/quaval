...
import CatalogViewer from "@/components/catalogs/CatalogViewer"
import { timkenCatalogs } from "@/config/catalogs"

type TimkenCatalogsPageProps = {
  params: Promise<{ locale: string }>
}

export default async function TimkenCatalogsPage({ params }: TimkenCatalogsPageProps) {
  const { locale } = await params
  const pageCount = timkenCatalogs.reduce((total, catalog) => total + catalog.pageCount, 0)
  ...
  return (
    <div className="min-h-screen bg-background text-text">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base leading-7 text-text-light sm:text-lg">
              Browse the Timken publications supplied by Quaval. Use the page and zoom controls
              to examine technical guidance and bearing tables online.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-black/10 bg-white px-5 py-4 shadow-sm">
            ...
            <div>
              <div className="text-2xl font-bold text-secondary">{pageCount}</div>
        ...
        <div className="mt-10">
          <CatalogViewer catalogs={timkenCatalogs} />
        </div>
      </div>
    </div>
  )
